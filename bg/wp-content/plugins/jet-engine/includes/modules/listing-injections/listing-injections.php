...
if ( ! class_exists( 'Jet_Engine_Module_Listing_Injections' ) ) {
	...
	class Jet_Engine_Module_Listing_Injections extends Jet_Engine_Module_Base {
		...
		public function maybe_inject_item( $content = false, $post, $i, $widget ) {
			...
			$injected_item = $this->get_injected_item( $settings, $post, $i );
		 ...
		 * @return [type] [description]
		 */
		public function get_injected_item( $settings, $post, $i ) {
			...
			foreach ( $items as $item ) {
				...
				$type = ! empty( $item['item_condition_type'] ) ? $item['item_condition_type'] : 'after_item';
		...
		public function add_settings( $widget ) {
			...
			$widget->add_control(
				...
				array(
					...
					'fields'    => array_values( $items_repeater->get_controls() ),
					'default'   => array(),
					'condition' => array(
