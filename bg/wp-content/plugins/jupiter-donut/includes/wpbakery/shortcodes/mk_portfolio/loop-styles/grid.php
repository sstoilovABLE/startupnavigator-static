...
$content_width = jupiter_donut_get_option( 'content_width' );
    ...
    switch ($view_params['column']) {
        ...
        case 2:
            if ($view_params['layout'] == 'full') {
                $width = round($grid_width / 2) - 25;
            } else {
                $width = round((($content_width / 100) * $grid_width) / 2) - 25;
            }
            $item_classes[] = 'two-column';
            break;
        case 3:
            if ($view_params['layout'] == 'full') {
                $width = round($grid_width / 3) - 20;
            } else {
                $width = round((($content_width / 100) * $grid_width) / 2) - 25;
            }
