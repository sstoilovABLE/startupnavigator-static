<?php
    ...
    $sortable_all_text = !empty($view_params['sortable_all_text']) ? esc_attr( $view_params['sortable_all_text'] ) : esc_html__( 'All', 'jupiter-donut' );
    ...
    <div class="mk-grid">
        <ul class="align-<?php echo esc_attr( $view_params['align'] ); ?>">
            <?php
                ...
                if ( $view_params['style'] == 'static' ) {
                    $data_filter_all = '*';
                } else {
                    $data_filter_all = esc_attr( $all_cats_items );
                }
                
                
            ?>
            <li><a class="<?php echo (empty($requested_term) ? 'current' : ''); ?>" data-filter="<?php echo $data_filter_all; ?>" href="<?php echo esc_url( get_permalink() ); ?>"><?php echo esc_html( $sortable_all_text ); ?></a></li>
