...
class Hubspot extends Action_Base {
	...
	public function update_controls( $widget ) {
		...
		$widget->add_control(
			'hubspot_portal_id',
			[
				'label' => __( 'Portal ID *', 'raven' ),
				'type' => Controls_Manager::TEXT,
				'description' => __( 'Enter the Hubspot Portal ID', 'raven' ) . ' ' . sprintf( '<a href="%s" target="_blank">%s</a>.', 'https://knowledge.hubspot.com/articles/kcs_article/account/access-your-hub-id-and-other-hubspot-accounts', __( 'More Info', 'raven' ) ),
			]
		);
		...
		$widget->add_control(
			'hubspot_form_id',
			[
				'label' => __( 'Form ID *', 'raven' ),
				'type' => Controls_Manager::TEXT,
				'description' => __( 'Enter the Hubspot Form ID', 'raven' ) . ' ' . sprintf( '<a href="%s" target="_blank">%s</a>.', 'https://knowledge.hubspot.com/articles/kcs_article/forms/find-your-form-guid', __( 'More Info', 'raven' ) ),
			]
		);
		...
		$widget->add_control(
			...
			[
				'label' => __( 'SFDC Campaign ID', 'raven' ),
				'type' => Controls_Manager::TEXT,
			]
		);
		...
		$widget->add_control(
			...
			[
				'label' => __( 'GoToWebinar key/ID', 'raven' ),
				'type' => Controls_Manager::TEXT,
			]
		);
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Submission timestamp', 'raven' ),
	...
	public static function run( $ajax_handler ) {
		...
		if ( 200 !== (int) wp_remote_retrieve_response_code( $response ) ) {
			$ajax_handler->add_response( 'admin_errors', __( 'Hubspot Action: Hubspot Webhook Error.', 'raven' ) );
		}
		...
		if ( ! empty( $response_body['errors'] ) ) {
			foreach ( $response_body['errors'] as $error ) {
				$ajax_handler->add_response( 'admin_errors', __( 'Hubspot Action: ', 'raven' ) . $error['message'] );
			}
		}
	}
