...
const namespaceAndEndpointMiddleware = (options, next) => {
  ...
  if (typeof options.namespace === "string" && typeof options.endpoint === "string") {
    ...
    if (endpointTrimmed) {
      path = namespaceTrimmed + "/bg/" + endpointTrimmed;
    } else {
...
const parseLinkHeader = (linkHeader) => {
  ...
  const match = linkHeader.match(/<([^>]+)>; rel="next"/bg/);
  return match ? {
    next: match[1]
  } : {};
};
...
function isMediaUploadRequest(options) {
  ...
  const isMediaEndpoint = !!options.path && options.path.indexOf("/bg/wp/v2/media") !== -1 || !!options.url && options.url.indexOf("/bg/wp/v2/media") !== -1;
