    ...
    if (orderedRecords?.length > 0 && templateType === "wp_template_part") {
      result.push({
        ...
        callback: ({ close }) => {
          if (isSiteEditor) {
            history.navigate(
              "/bg/pattern?postType=wp_template_part&categoryId=all-parts"
            );
          } else {
            document.location = (0,external_wp_url_namespaceObject.addQueryArgs)(
              "site-editor.php",
  ...
  const commands = (0,external_wp_element_namespaceObject.useMemo)(() => {
    ...
    if (canCreateTemplate && isBlockBasedTheme) {
      ...
      result.push({
        ...
        callback: ({ close }) => {
          if (isSiteEditor) {
            history.navigate("/bg/navigation");
          } else {
            document.location = (0,external_wp_url_namespaceObject.addQueryArgs)(
              "site-editor.php",
              {
    ...
    return [
      {
        ...
        callback: ({ close }) => {
          close();
          if (isSiteEditor) {
            history.navigate("/bg/styles?section=/css");
          } else {
            document.location = (0,external_wp_url_namespaceObject.addQueryArgs)(
              "site-editor.php",
              {
