...
function getUserPermissionCacheKey(action, resource, id) {
  const key = (typeof resource === "object" ? [action, resource.kind, resource.name, resource.id] : [action, resource, id]).filter(Boolean).join("/bg/");
  return key;
}
...
const saveEntityRecord = (kind, name, record, {
  ...
  try {
    ...
    let hasError = false;
    try {
      const path = `${entityConfig.baseURL}${recordId ? "/bg/" + recordId : ""}`;
...
const rootEntitiesConfig = [
  ...
  },
  {
    name: "menuLocation",
    kind: "root",
    baseURL: "/bg/wp/v2/menu-locations",
    baseURLParams: { context: "edit" },
  ...
  },
  {
    ...
    name: "globalStyles",
    kind: "root",
    baseURL: "/bg/wp/v2/global-styles",
    baseURLParams: { context: "edit" },
    plural: "globalStylesVariations",
    // Should be different from name.
    getTitle: () => (0,external_wp_i18n_.__)("Custom Styles"),
    getRevisionsUrl: (parentId, revisionId) => `/wp/v2/global-styles/${parentId}/revisions${revisionId ? "/bg/" + revisionId : ""}`,
    supportsPagination: true
  },
  {
...
async function loadPostTypeEntities() {
  const postTypes = await external_wp_apiFetch_default()({
    path: "/bg/wp/v2/types?context=view"
  });
  return Object.entries(postTypes ?? {}).map(([name, postType]) => {
    ...
    const entity = {
      ...
      supportsPagination: true,
      getRevisionsUrl: (parentId, revisionId) => `/${namespace}/${postType.rest_base}/${parentId}/revisions${revisionId ? "/bg/" + revisionId : ""}`,
...
async function loadTaxonomyEntities() {
  const taxonomies = await external_wp_apiFetch_default()({
    path: "/bg/wp/v2/taxonomies?context=view"
  });
...
const getAuthors = (query) => async ({ dispatch }) => {
  const path = (0,external_wp_url_.addQueryArgs)(
    "/bg/wp/v2/users/?who=authors&per_page=100",
    query
  );
...
const getCurrentUser = () => async ({ dispatch }) => {
  const currentUser = await external_wp_apiFetch_default()({ path: "/bg/wp/v2/users/me" });
...
const getEntityRecord = (kind, name, key = "", query) => async ({ select, dispatch, registry, resolveSelect }) => {
  ...
  try {
    ...
    const path = (0,external_wp_url_.addQueryArgs)(
      entityConfig.baseURL + (key ? "/bg/" + key : ""),
      {
...
const getEmbedPreview = (url) => async ({ dispatch }) => {
  try {
    const embedProxyResponse = await external_wp_apiFetch_default()({
      path: (0,external_wp_url_.addQueryArgs)("/bg/oembed/1.0/proxy", { url })
    });
...
const canUser = (requestedAction, resource, id) => async ({ dispatch, registry, resolveSelect }) => {
  ...
  if (typeof resource === "object") {
    ...
    resourcePath = entityConfig.baseURL + (resource.id ? "/bg/" + resource.id : "");
  } else {
    resourcePath = `/wp/v2/${resource}` + (id ? "/bg/" + id : "");
  }
  let response;
  try {
...
const getBlockPatternCategories = () => async ({ dispatch }) => {
  const categories = await external_wp_apiFetch_default()({
    path: "/bg/wp/v2/block-patterns/categories"
  });
...
const getNavigationFallbackId = () => async ({ dispatch, select, registry }) => {
  const fallback = await external_wp_apiFetch_default()({
    path: (0,external_wp_url_.addQueryArgs)("/bg/wp-block-editor/v1/navigation-fallback", {
      _embed: true
    })
  });
...
const getDefaultTemplateId = (query) => async ({ dispatch, registry, resolveSelect }) => {
  const template = await external_wp_apiFetch_default()({
    path: (0,external_wp_url_.addQueryArgs)("/bg/wp/v2/templates/lookup", query)
  });
...
async function fetchLinkSuggestions(search, searchOptions = {}, editorSettings = {}) {
  ...
  if (!type || type === "post") {
    queries.push(
      external_wp_apiFetch_default()({
        path: (0,external_wp_url_.addQueryArgs)("/bg/wp/v2/search", {
          search,
          page,
          per_page: perPage,
          type: "post",
  ...
  if (!type || type === "attachment") {
    queries.push(
      external_wp_apiFetch_default()({
        path: (0,external_wp_url_.addQueryArgs)("/bg/wp/v2/media", {
          search,
          page,
          per_page: perPage
        })
...
const fetchUrlData = async (url, options = {}) => {
  const endpoint = "/bg/wp-block-editor/v1/url-details";
  const args = {
...
async function fetchBlockPatterns() {
  const restPatterns = await external_wp_apiFetch_default()({
    path: "/bg/wp/v2/block-patterns/patterns"
  });
  if (!restPatterns) {
    return [];
  }
