...
if ( ! class_exists( 'Jet_Engine_Dynamic_Functions' ) ) {
	...
	class Jet_Engine_Dynamic_Functions {
		...
		public function call_sql_function( $data ) {
			...
			$final_query = "SELECT $query FROM $table $where;";
				...
				switch ( $data_source['context'] ) {
					...
					case 'current_term':
						...
						if ( $term_id ) {
							...
							$final_query = "SELECT $query FROM $table INNER JOIN $term_relationships ON $table.post_id = $term_relationships.object_id $where AND $term_relationships.term_taxonomy_id = $term_id";
						}
					...
					case 'current_user':
						...
						if ( $user_id ) {
							$posts = $wpdb->posts;
							$final_query = "SELECT $query FROM $table INNER JOIN $posts ON $table.post_id = $posts.ID $where AND $posts.post_author = $user_id";
						}
