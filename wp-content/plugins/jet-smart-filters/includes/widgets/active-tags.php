...
class Jet_Smart_Filters_Active_Tags_Widget extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} ' . $css_scheme['tags'] => 'align-items: {{VALUE}};',
				),
				'condition' => array(
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} ' . $css_scheme['tags'] => 'justify-content: {{VALUE}};',
				),
				'condition' => array(
					'tags_position' => 'row'
				)
	...
	public function render_tags_sample() {
		...
		$clear_title = $settings['clear_item_label'];
	...
	protected function render() {
		...
		printf(
			'<div class="%1$s jet-active-tags" data-tags-label="%5$s" data-clear-item-label="%6$s" data-apply-provider="%2$s" data-apply-type="%3$s" data-query-id="%4$s">',
			$base_class,
			$settings['content_provider'],
			$settings['apply_type'],
			$query_id,
			$settings['tags_label'],
			$clear_item ? $settings['clear_item_label'] : false
		);

		jet_smart_filters()->render->set_rendered( $settings['content_provider'], $query_id );
