...
class Mk_Send_Mail {
	...
	function __construct() {
		add_action(
			'wp_ajax_nopriv_mk_contact_form', array(
				&$this,
				'send_form',
			)
		);
		add_action(
			'wp_ajax_mk_contact_form', array(
				&$this,
				'send_form',
			)
		);
	...
	function send_form() {
		...
		if ( $send_to === '' || $email === '' || $content === '' || $name === '' ) {
			$error .= __( ' - One of the fields are empty' , 'jupiter-donut' ) . '<br>';
		}

		if ( ! empty( $email ) && filter_var( $email, FILTER_VALIDATE_EMAIL ) === false ) {
			$error .= __( " - 'Email' field seems not to be an email." , 'jupiter-donut' ) . '<br>';
		}

		if ( ! empty( $send_to ) && filter_var( $send_to, FILTER_VALIDATE_EMAIL ) === false ) {
			$error .= __( " - 'Send To' field seems not to be an email." , 'jupiter-donut' ) . '<br>';
		}

		if ( $error == false ) {
			$subject = sprintf( __( '%1$s\'s message from %2$s', 'jupiter-donut' ), $sitename, $name );

				$body = __( 'Site: ', 'jupiter-donut' ) . $sitename . ' (' . $siteurl . ')' . "\n\n";

				$body .= __( 'Name: ', 'jupiter-donut' ) . $name . ' ' . $last_name . "\n\n";

				$body .= __( 'Email: ', 'jupiter-donut' ) . $email . "\n\n";

			if ( ! empty( $phone ) ) {
				$body .= __( 'Phone Number: ', 'jupiter-donut' ) . $phone . "\n\n";
			}

			if ( ! empty( $website ) ) {
				$body .= __( 'Website: ', 'jupiter-donut' ) . $website . "\n\n";
			}

				$body .= __( 'Messages: ', 'jupiter-donut' ) . $content;
			...
			if ( wp_mail( $send_to, $subject, $body, $headers ) ) {
				echo json_encode(
					array(
						'action_Status' => true,
						'message' => __( 'Email sent!', 'jupiter-donut' ),
					)
				);
			} else {
					echo json_encode(
						array(
							'action_Status' => false,
							'message' => __( 'Email could not be sent! There is an issue with mail server!', 'jupiter-donut' ),
						)
					);
			}
		} else {
	...
	function from_email() {
		...
		if ( ($_SERVER['REMOTE_ADDR'] === '127.0.0.1' || $_SERVER['REMOTE_ADDR'] === 'localhost' || $_SERVER['REMOTE_ADDR'] === '::1') ? 1 : 0 ) {
			return $admin_email;
		}
