...
class Mailchimp extends Action_Base {
	...
	public function update_controls( $widget ) {
		...
		$widget->add_control(
			'mailchimp_api_key',
			[
				'label' => __( 'Custom API Key', 'raven' ),
				'type' => 'text',
				'condition' => [
					'mailchimp_api_key_source' => 'custom',
				],
				'description' => __( 'Enter your Mailchimp API key for only this form.', 'raven' ),
			]
		);
		...
		$repeater->add_control(
			'mailchimp_remote_field', [
				'label' => __( 'Mailchimp Field', 'raven' ),
				'type' => 'select',
				'label_block' => false,
			]
		);
	...
	public static function run( $ajax_handler ) {
		...
		try {
			...
			if ( ! $handler->success() ) {
				return $ajax_handler->add_response(
					'admin_errors',
					__( 'MailChimp Audience ID is not valid.', 'raven' )
				);
			}
	...
	public static function get_list_details( $ajax_handler, array $params = [] ) {
		if ( empty( $params['mailchimp_list'] ) ) {
			return $ajax_handler->set_success( false )
				->add_response( 'error', __( 'Mailchimp Audience ID is missing.', 'raven' ) );
		}
